...
# Vendored as a child module of the capstone root. The root (../versions.tf)
# owns the terraform{} + provider "aws" blocks; a child module must not. The
# archive provider requirement is declared in the root's versions.tf.
...
resource "aws_dynamodb_table" "intake" {
  ...
  # GAP-02 CLOSED: customer-managed CMK encryption. The capstone root passes
  # the acme-submissions CMK via var.submissions_kms_key_arn.
  server_side_encryption {
    enabled     = true
    kms_key_arn = var.submissions_kms_key_arn
  }
}
...
# GAP-07 CLOSED: the starter's broad `dynamodb:*` / `s3:*` inline policy is
# removed at source. The capstone attaches a least-privilege replacement in
# ../overrides_starter.tf (aws_iam_role_policy.lambda_dynamodb_scoped +
# lambda_s3_scoped), which the Rego gate verifies.
